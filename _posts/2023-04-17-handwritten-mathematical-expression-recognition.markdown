---
layout: post
title: "Beyond Pen and Paper: Decoding Handwritten Equations with Transformers"
description: Learn how to turn handwritten math into LaTeX using Transformers and the CROHME 2023 dataset.
image: /assets/images/handwritten-mathematical-expression-recognition/splash.png
authors: [tlyleung]
permalink: handwritten-mathematical-expression-recognition
---
...
```python
pe = PositionalEncoding1D(d_model=256, max_len=100).pe
plt.matshow(pe)
ax = plt.gca()
...
![png](/assets/images/handwritten-mathematical-expression-recognition/handwriting_31_0.png){: .multiply }
    


### 2D Positional Encoding

For the 2D positional encoding, we use a more complex visualisation. We create a 2D positional encoding with `d_model` set to 32 and `max_len` to 100. Each subplot below show different layers (or dimensions) of the 2D positional encoding, allowing you to see how each dimension encodes 2D positional information differently.


```python
pe = PositionalEncoding2D(d_model=32, max_len=100).pe
...
for i in range(3):
    for j in range(3):
        ...
        ax.set_axis_off()
        frame_idx = (i * 3 + j)
        cax = ax.matshow(pe[:, :, frame_idx], cmap='viridis')
        ax.set_title(f"Dimension {frame_idx}", y=1.02)
...
![png](/assets/images/handwritten-mathematical-expression-recognition/handwriting_33_0.png){: .multiply }
...
```python
# Visualise the first example from the batch
id_to_token = datamodule.tokenizer.id_to_token
tgt_sample = tgt[0][:12]
mask_sample = tgt_mask[:12, :12].clone()
mask_sample[mask_sample == float("-inf")] = 1


fig, ax = plt.subplots()

# Display the attention mask; use a colormap that clearly shows masked areas
cax = ax.imshow(mask_sample, aspect='auto', cmap='binary', alpha=0.5)

# Add token IDs as text annotations
for i in range(len(tgt_sample)):
    for j in range(len(tgt_sample)):
        color = "white" if j > i else "black"
        id = tgt_sample[j].item()
        text = ax.text(j, i, id_to_token[id], ha="center", va="center", color=color, size=8)
...
![png](/assets/images/handwritten-mathematical-expression-recognition/handwriting_38_0.png){: .multiply }
