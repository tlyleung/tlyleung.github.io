...
Classification models predict the likelihood that something belongs to a category. If there are only two possible categories, it's binary classification, while multi-class classification is when there are more than 2 categories.
...
- Logistic Regression:  models probability of binary outcome by using a linear combination of features.y = 1x1 +…+ nxn + 
  - Pros: fast, efficient, works well for linearly separable data, interpretable and easy to understand
  - Cons: cannot solve non-linear problems, multi-collinearity
- Decision tree: split data using features into tree structure
  - Pros: fast, no data prep required, interpretable
  - Cons: Non-optimal decision boundary, prone to overfitting 


K-nearest neighbours: regression and classification, calculates the k-nearest neighbours to point and averages the label
Support vector machines: find hyperplane that separates classes by the widest margin
Random forests: 
XGBoost: combine many weak learns into a strong learner
Catboost
...
<img src="/assets/images/machine-learning-cheatsheet/transformer.svg" alt="Transfomer" class="img-fluid">
...
Semi-supervised learning involves learning from a training set that contains a small amount of labelled data and a large amount of unlabeled data.
...
K-means clustering
Dimensionality reduction: minimize the computational resources required to process, store, and analyze data, help mitigate curse of dimensionality, make data visualisation more feasible
